...
export const deleteStory = (req, res) => {
  ...
  jwt.verify(token, "secretkey", (err, userInfo) => {
    ...
    const getStoriesOfUser = "SELECT * FROM stories WHERE userId = ?"
    db.query(getStoriesOfUser, [userInfo.id], (err, data) => {
      if (err) return res.status(500).json(err);
      if (!data.length) return res.status(400).json("User don't have storie!");
    })

    const q = "DELETE FROM stories WHERE id=?";
    db.query(q, [req.params.storieId], (err, data) => {
      if (err) return res.status(500).json(err);
